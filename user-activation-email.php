...
class UserActivationEmail {
	...
	public function add_login_field() {
		?>
		<p>
			<label for="activation-code"><?php echo __( 'Activation Code (New User Only)', 'user-activation-email' ); ?>
				<br>
				<input type="text" id="activation-code" class="input" name="activation-code" tabindex="20"
							 value="<?php if ( isset( $_GET['uae-key'] ) ) {
								 echo $_GET['uae-key'];
							 } ?>">
			</label>
		</p>
	...
	public function add_user_profile_fields( $user ) {
		if ( current_user_can( 'manage_options', $user->ID ) ) {
			...
			<table class="form-table">
				<tr>
					<th><label
								for="activation-code"><?php _e( 'Activation Code (10 characters)', 'user-activation-email' ); ?></label>
					</th>
					<td>
						<input type="text" id="activation-code" name="activation-code"
									 value="<?php echo esc_attr( get_the_author_meta( self::user_meta, $user->ID ) ); ?>"
									 class="regular-text">
					</td>
				</tr>
			</table>
...
if ( ! function_exists( 'wp_new_user_notification' ) ) :
	...
	function wp_new_user_notification( $user_id, $plaintext_pass = '' ) {
		...
		$message .= sprintf( __( 'Activation Code: %s', 'user-activation-email' ), site_url( $activation_code ) ) . "\r\n\n";

		@wp_mail( $user_email, sprintf( __( '[%s] New User Registration', 'user-activation-email' ), get_option( 'blogname' ) ), $message );
